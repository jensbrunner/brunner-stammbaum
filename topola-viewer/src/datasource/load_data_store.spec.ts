...
import {loadFromUrl, loadGedcom} from './load_data';
...
describe('loadFromUrl() session cache', () => {
  let sessionStorageMock: {[key: string]: string};
  const url = 'https://example.org/tree/brunner.ged';
  ...
  beforeEach(() => {
    ...
    Object.defineProperty(global, 'window', {
      configurable: true,
      value: {
        location: {href: 'https://viewer.example.org/'},
        fetch: jest.fn().mockResolvedValue({
          status: 200,
          blob: () => Promise.resolve(new Blob([MINIMAL_GEDCOM])),
        }),
      },
    });
  });

  it('ignores stale cached data prepared without the URL media base', async () => {
    sessionStorageMock[url] = JSON.stringify({
      chartData: {indis: [], fams: []},
      gedcom: {indis: {}, fams: {}, other: {}},
    });

    const data = await loadFromUrl(url, false);

    expect(window.fetch).toHaveBeenCalledWith(url);
    expect(data.mediaBaseUrl).toBe('https://example.org/tree/');
    expect(data.chartData.indis).toHaveLength(1);
  });
});
