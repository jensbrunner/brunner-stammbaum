...
async function prepareData(
  ...
  const data = await convertGedcom(
    gedcom,
    images || new Map(),
    onProgress,
    mediaBaseUrl,
...
function getMediaBaseUrl(url: string): string | undefined {
  try {
...
export async function loadFromUrl(
  ...
  const mediaBaseUrl = getMediaBaseUrl(url);
  try {
    const cachedData = sessionStorage.getItem(url);
    if (cachedData) {
      const parsedData = JSON.parse(cachedData) as TopolaData;
      if (parsedData.mediaBaseUrl === mediaBaseUrl) {
        return parsedData;
      }
