...
export function resolveFileUrl(
  ...
  if (mediaBaseUrl && !/^[a-z]:\//i.test(normalizedUrl)) {
    try {
      return new URL(normalizedUrl, mediaBaseUrl).href;
    } catch (e) {
      // Fall through to returning the normalized local path.
    }
  }
...
function filterImage(
  ...
  indi.images.forEach((image) => {
    const resolvedUrl = resolveFileUrl(image.url, images, mediaBaseUrl);
...
function filterImages(
  ...
  const newIndis = gedcom.indis.map((indi) =>
    filterImage(indi, images, mediaBaseUrl),
  );
  return Object.assign({}, gedcom, {indis: newIndis});
}
...
export async function convertGedcom(
  ...
  const chartData = filterImages(normalizeGedcom(json), images, mediaBaseUrl);
